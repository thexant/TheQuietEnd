...
• Corridors have two or more **stable entry/exit regions** (informally referred to as "positions" or "points").
...
• **Gates** are massive human-built structures that anchor Corridors in place, forcing them open and stable for transit.

• Without a gate, Corridors become **inaccessible**, unpredictable, or dangerous to enter.

• Gates are resource-intensive, immobile, and require continual power and upkeep.

• If a Corridor **shifts or closes**, a gate may become **useless**, requiring recovery and salvage or abandonment.

**Shifting and Collapse**

• Corridors naturally **shift** between a set of rough positions (usually 8–12). The shift is unpredictable, but known routes may return over time.

• Even when a Corridor shifts back to a previous position, it never returns to the **exact coordinates**. Gates must be **relocated** within the system to match the new offset.

• A **collapse** is different from a shift. When a Corridor collapses:

    ◦ One of its former positions is permanently lost.

    ◦ It either **adds a new position** to its rotation or becomes a **new Corridor entirely**, with different entry/exit points.
